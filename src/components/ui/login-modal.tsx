"use client";

import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import { motion, AnimatePresence } from "motion/react";
import { GraduationCap, Briefcase, AlertCircle, ArrowRight, Loader2 } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { createClient } from "@/lib/supabase/client";
import { cn } from "@/lib/utils";

interface LoginModalProps {
  children: React.ReactNode;
  className?: string;
}

export function LoginModal({ children, className }: LoginModalProps) {
  const router = useRouter();
  const [role, setRole] = useState<"student" | "recruiter">("student");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [isGoogleLoading, setIsGoogleLoading] = useState(false);
  const [isGithubLoading, setIsGithubLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Pre-warm destination route chunks so transition takes 0ms
  useEffect(() => {
    router.prefetch("/onboarding");
    router.prefetch("/dashboard");
    router.prefetch("/recruiter");
  }, [router]);

  const rememberRoleChoice = () => {
    if (role === "recruiter") {
      document.cookie = `post_login_next=${encodeURIComponent("/recruiter-signup")}; path=/; max-age=600; samesite=lax`;
      return;
    }
    document.cookie = "post_login_next=; path=/; max-age=0; samesite=lax";
  };

  const handleGoogleLogin = async () => {
    setError(null);
    setIsGoogleLoading(true);
    const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
    if (!supabaseUrl || supabaseUrl.includes("your-project-ref") || supabaseUrl.includes("mock-project")) {
      setError("Please set your real NEXT_PUBLIC_SUPABASE_URL and ANON_KEY in the .env file to enable Google Login.");
      setIsGoogleLoading(false);
      return;
    }
    try {
      rememberRoleChoice();
      const supabase = createClient();
      const callback = `${window.location.origin}/auth/callback`;
      await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: callback,
        },
      });
    } catch (err: any) {
      setError(err?.message || "Failed to initiate Google login");
      setIsGoogleLoading(false);
    }
  };

  const handleGithubLogin = async () => {
    setError(null);
    setIsGithubLoading(true);
    const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
    if (!supabaseUrl || supabaseUrl.includes("your-project-ref") || supabaseUrl.includes("mock-project")) {
      setError("Please set your real NEXT_PUBLIC_SUPABASE_URL and ANON_KEY in the .env file to enable GitHub Login.");
      setIsGithubLoading(false);
      return;
    }
    try {
      rememberRoleChoice();
      const supabase = createClient();
      const callback = `${window.location.origin}/auth/callback`;
      await supabase.auth.signInWithOAuth({
        provider: "github",
        options: {
          redirectTo: callback,
        },
      });
    } catch (err: any) {
      setError(err?.message || "Failed to initiate GitHub login");
      setIsGithubLoading(false);
    }
  };

  const handleEmailLogin = async () => {
    setError(null);
    setIsLoading(true);
    const supabase = createClient();
    rememberRoleChoice();

    const { error: signInError } = await supabase.auth.signInWithPassword({
      email,
      password,
    });

    if (signInError) {
      if (signInError.message.includes("Invalid login credentials")) {
        const { error: signUpError } = await supabase.auth.signUp({
          email,
          password,
          options: {
            emailRedirectTo: `${window.location.origin}/auth/callback`,
          },
        });
        if (signUpError) {
          setError(signUpError.message);
        } else {
          setError("Check your email for the confirmation link!");
        }
      } else {
        setError(signInError.message);
      }
    } else {
      window.location.assign(role === "recruiter" ? "/recruiter-signup" : "/dashboard");
      return;
    }
    setIsLoading(false);
  };

  return (
    <Dialog>
      <DialogTrigger render={<div className={cn("inline-block", className)} />} nativeButton={false}>
        {children}
      </DialogTrigger>

      <DialogContent className="sm:max-w-md bg-[#FAF9F6] border-2 border-zinc-900 rounded-3xl p-5 sm:p-8 shadow-[6px_6px_0px_0px_#18181B] sm:shadow-[8px_8px_0px_0px_#18181B] text-zinc-900 select-none">
        <DialogHeader className="flex flex-col items-start sm:text-left mt-0 gap-1.5">
          <div className="flex items-center gap-2">
            <span className="font-doodle text-xl sm:text-2xl text-blue-600 font-bold -rotate-1">
              ✨ Sign in to Minskey
            </span>
          </div>

          <DialogTitle className="text-2xl sm:text-3xl font-black tracking-tight text-zinc-950">
            Welcome Back
          </DialogTitle>

          {/* Student / Recruiter Tactile Switch */}
          <div className="relative w-full p-1 rounded-2xl bg-zinc-100 border-2 border-zinc-900 grid grid-cols-2 gap-1 shadow-xs my-2">
            <button
              type="button"
              onClick={() => setRole("student")}
              className={cn(
                "relative z-10 py-2 px-4 rounded-xl text-xs font-black flex items-center justify-center gap-1.5 transition-all duration-200 cursor-pointer select-none",
                role === "student"
                  ? "bg-white text-zinc-950 border-2 border-zinc-900 shadow-[2px_2px_0px_0px_#18181B]"
                  : "text-zinc-600 hover:text-zinc-950"
              )}
            >
              <GraduationCap className="h-4 w-4 text-blue-600" />
              <span>Student</span>
            </button>

            <button
              type="button"
              onClick={() => setRole("recruiter")}
              className={cn(
                "relative z-10 py-2 px-4 rounded-xl text-xs font-black flex items-center justify-center gap-1.5 transition-all duration-200 cursor-pointer select-none",
                role === "recruiter"
                  ? "bg-white text-zinc-950 border-2 border-zinc-900 shadow-[2px_2px_0px_0px_#18181B]"
                  : "text-zinc-600 hover:text-zinc-950"
              )}
            >
              <Briefcase className="h-4 w-4 text-purple-600" />
              <span>Recruiter</span>
            </button>
          </div>

          <AnimatePresence mode="wait">
            <motion.div
              key={role}
              initial={{ opacity: 0, y: 3 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -3 }}
              transition={{ duration: 0.15 }}
            >
              <DialogDescription className="text-zinc-700 text-xs leading-relaxed max-w-sm font-medium">
                {role === "student"
                  ? "Turn GitHub activity, certifications, and achievements into an evidence-backed skill passport."
                  : "Discover verified student talent, evaluate skill passports, and recruit with proof."}
              </DialogDescription>
            </motion.div>
          </AnimatePresence>
        </DialogHeader>

        <div className="flex flex-col gap-4 mt-2">
          {error && (
            <div role="alert" className="flex items-center gap-2 text-rose-950 text-xs bg-rose-50 p-2.5 rounded-xl border-2 border-rose-600 font-bold">
              <AlertCircle className="w-4 h-4 text-rose-600 shrink-0" />
              <span>{error}</span>
            </div>
          )}

          <div className="flex flex-col gap-1.5">
            <Label htmlFor="modal-email" className="text-zinc-950 font-black text-xs uppercase tracking-wide">
              Email Address
            </Label>
            <Input
              id="modal-email"
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder="you@college.edu"
              className="h-11 bg-white border-2 border-zinc-900 text-zinc-950 placeholder:text-zinc-500 focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 rounded-xl text-sm transition-all"
            />
          </div>

          <div className="flex flex-col gap-1.5">
            <Label htmlFor="modal-password" className="text-zinc-950 font-black text-xs uppercase tracking-wide">
              Password
            </Label>
            <Input
              id="modal-password"
              type="password"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              placeholder="Enter your password"
              className="h-11 bg-white border-2 border-zinc-900 text-zinc-950 placeholder:text-zinc-500 focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 rounded-xl text-sm transition-all"
            />
          </div>

          <Button
            onClick={handleEmailLogin}
            disabled={isLoading}
            className="w-full h-12 text-sm font-black mt-1 bg-blue-600 hover:bg-blue-700 text-white rounded-xl border-2 border-zinc-900 shadow-[3px_3px_0px_0px_#18181B] active:translate-x-[1px] active:translate-y-[1px] transition-all cursor-pointer flex items-center justify-center gap-2"
          >
            <span>{isLoading ? "Signing in..." : `Sign in as ${role === "student" ? "Student" : "Recruiter"}`}</span>
            <ArrowRight className="w-4 h-4" />
          </Button>

          <div className="relative my-1">
            <div className="absolute inset-0 flex items-center">
              <span className="w-full border-t-2 border-dashed border-zinc-300" />
            </div>
            <div className="relative flex justify-center text-xs">
              <span className="bg-[#FAF9F6] px-3 font-doodle text-base font-bold text-zinc-700">
                or sign in with
              </span>
            </div>
          </div>

          <div className={cn("grid gap-3", role === "student" ? "grid-cols-2" : "grid-cols-1")}>
            <Button
              onClick={handleGoogleLogin}
              disabled={isLoading || isGoogleLoading || isGithubLoading}
              variant="outline"
              size="lg"
              className="w-full gap-2.5 h-11 text-xs font-black bg-white border-2 border-zinc-900 hover:bg-zinc-50 text-zinc-950 rounded-xl shadow-[3px_3px_0px_0px_#18181B] active:translate-y-[2px] transition-all cursor-pointer"
            >
              {isGoogleLoading ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin text-blue-600" />
                  <span>Connecting...</span>
                </>
              ) : (
                <>
                  <svg className="h-4 w-4 shrink-0" viewBox="0 0 24 24">
                    <path
                      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                      fill="#4285F4"
                    />
                    <path
                      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                      fill="#34A853"
                    />
                    <path
                      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                      fill="#FBBC05"
                    />
                    <path
                      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                      fill="#EA4335"
                    />
                  </svg>
                  <span>Google</span>
                </>
              )}
            </Button>

            {role === "student" && (
              <Button
                onClick={handleGithubLogin}
                disabled={isLoading || isGoogleLoading || isGithubLoading}
                variant="outline"
                size="lg"
                className="w-full gap-2.5 h-11 text-xs font-black bg-white border-2 border-zinc-900 hover:bg-zinc-50 text-zinc-950 rounded-xl shadow-[3px_3px_0px_0px_#18181B] active:translate-y-[2px] transition-all cursor-pointer"
              >
                {isGithubLoading ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin text-zinc-900" />
                    <span>Connecting...</span>
                  </>
                ) : (
                  <>
                    <svg className="h-4 w-4 shrink-0 text-zinc-950 fill-current" viewBox="0 0 24 24">
                      <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                    </svg>
                    <span>GitHub</span>
                  </>
                )}
              </Button>
            )}
          </div>

          {/* Quick Demo Bypass */}
          <div className="pt-2 border-t border-dashed border-zinc-300">
            <button
              type="button"
              onClick={() => window.location.assign(role === "recruiter" ? "/recruiter" : "/dashboard")}
              className="w-full py-2.5 rounded-xl bg-amber-100 hover:bg-amber-200 border-2 border-zinc-900 text-amber-950 font-black text-xs shadow-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
            >
              <span>⚡ Instant Demo Access ({role === "recruiter" ? "Recruiter Portal" : "Student Dashboard"})</span>
            </button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
  );
}
