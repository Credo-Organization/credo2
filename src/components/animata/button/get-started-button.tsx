...
interface IGetStartedButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  text?: string;
  className?: string;
}
...
export default function GetStartedButton({
  ...
  return (
    <div className="min-h-12 w-auto inline-flex items-center justify-center">
      <button
        type="button"
        {...props}
        className={cn(
          "group/start flex h-12 min-w-40 items-center justify-center gap-3 rounded-lg bg-amber-100 px-5 py-2 font-bold transition-colors duration-100 ease-in-out hover:bg-orange-600 cursor-pointer shadow-md",
          className,
        )}
      >
        <span
