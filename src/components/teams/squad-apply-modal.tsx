      ...
      <DialogContent className="sm:max-w-xl bg-white border border-stone-200 text-stone-900 p-6 rounded-3xl shadow-2xl backdrop-blur-2xl">
        <DialogHeader className="space-y-2 text-left">
          <div className="flex items-center gap-2">
            <div className="px-2.5 py-0.5 rounded-full bg-navy-500/10 border border-navy-500/20 text-navy-700 text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5">
        ...
        {isSuccess ? (
          <div className="py-12 flex flex-col items-center justify-center text-center space-y-3">
            <div className="w-14 h-14 rounded-2xl bg-navy-500/20 border border-navy-500/40 flex items-center justify-center text-navy-700 animate-bounce">
          ...
          <form onSubmit={handleApply} className="space-y-4 pt-2">
                ...
                {squad.open_roles?.map((role: string) => (
                  <button
                    ...
                    className={`px-3 py-2 rounded-xl text-xs font-medium text-left border transition-all cursor-pointer ${
                      selectedRole === role
                        ? "bg-stone-100 border-navy-400 text-stone-900 shadow-sm"
            ...
            <div className="p-3.5 rounded-2xl bg-stone-50 border border-stone-200 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-stone-500 uppercase tracking-wider flex items-center gap-1.5">
                  <Terminal className="w-3.5 h-3.5 text-navy-700" />
                  Attached Proof Vectors
                </span>
                <span className="text-[10px] text-navy-700 font-mono">
                  did:cdy:ed25519:7421...
                </span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                <span className="px-2 py-0.5 rounded bg-navy-500/10 border border-navy-500/20 text-[10px] font-mono text-navy-700">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full bg-stone-100 border border-stone-200 rounded-xl px-3.5 py-2.5 text-xs text-stone-900 placeholder:text-stone-500 focus:outline-none focus:border-navy-400 transition-colors resize-none"
              />
            </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-stone-200">
              ...
              <Button
                ...
                className="h-9 px-5 text-xs font-semibold bg-navy-900 hover:bg-navy-800 text-white rounded-xl shadow-lg shadow-navy-900/20 transition-all flex items-center gap-1.5 cursor-pointer"
              >
                {isSubmitting ? (
                  "Sending Handshake..."
                ) : (
